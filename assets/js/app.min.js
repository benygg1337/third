...
let slider_header = new Swiper('.menu__list', {
	observer: true,
	observeParents: true,
	slidesPerView: 'auto',
	spaceBetween: 0,
...
document.querySelector('.book__form').addEventListener('submit', function (event) {
	...
	fetch('/wp-content/themes/third/submit_wish.php', {
		method: 'POST',
		body: formData
	})
	...
	.then(data => {
		if (data.success) {
			// Пожелание успешно создано, выводим сообщение об успешной отправке
			document.getElementById('form-message').textContent = data.message;

			// Через 2 секунды выполняем перенаправление на страницу, где была форма
			setTimeout(function () {
				window.location.href = window.location.href; // Перенаправляем на текущую страницу
			}, 2000);
		} else {
			// Возникла ошибка при создании пожелания, выводим сообщение об ошибке
			document.getElementById('form-message').textContent = data.message;
		}
	})
	.catch(error => {
